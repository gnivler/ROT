...
public class RealmOfThronesGameModeManager : MBGameManager
{
    ...
    protected override void DoLoadingForGameManager(GameManagerLoadingSteps gameManagerLoadingStep, out GameManagerLoadingSteps nextStep)
    {
        ...
        switch (gameManagerLoadingStep)
        {
            ...
            case GameManagerLoadingSteps.WaitSecondStep:
                if (!_loadingSavedGame) StartNewGame();
            ...
            {
                var flag = true;
                foreach (var subModule in Module.CurrentModule.SubModules) flag = flag && subModule.DoLoading(Game.Current);
    ...
    public override void OnLoadFinished()
    {
        if (!_loadingSavedGame)
        {
            ...
            if (!Game.Current.IsDevelopmentMode) LaunchSandboxCharacterCreation();
        }
        else
        {
            if (CampaignSiegeTestStatic.IsSiegeTestBuild) CampaignSiegeTestStatic.DisableSiegeTest();
            ...
            string text = null;
            if (Game.Current.GameStateManager.ActiveState is MapState) text = ((MapState)Game.Current.GameStateManager.ActiveState).GameMenuId;
            ...
            foreach (var item in Settlement.All) item.Party.Visuals.RefreshLevelMask(item.Party);
    ...
    {
        Game.Current.GameStateManager.CleanAndPushState(Game.Current.GameStateManager.CreateState<CharacterCreationState>(new ROTCharacterCreationContent()));
    }
    ...
    {
        if (Campaign.Current?.PreviouslyUsedModules == null) return null;
        string[] moduleNames = SandBoxManager.Instance.ModuleManager.ModuleNames;
        var list = new List<(string, string)>();
        foreach (var previouslyUsedModule in Campaign.Current.PreviouslyUsedModules)
        {
            var module = previouslyUsedModule;
            var flag = moduleNames.FindIndex(x => x == module) != -1;
